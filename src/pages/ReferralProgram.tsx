...
const ReferralProgram = () => {
  ...
  const loadReferralData = async () => {
    try {
      ...
      const {
        data: rewards,
        error: rewardsError
      } = await supabase.from('referral_rewards').select(`
          credits_awarded,
          created_at,
          referred_id
        `).eq('referrer_id', user!.id).eq('status', 'completed').order('created_at', {
        ascending: false
      });
      if (rewardsError) {
      ...
      const recentWithUsernames = await Promise.all((rewards || []).slice(0, 5).map(async reward => {
        const {
          data: profile
        } = await supabase.from('profiles').select('username').eq('user_id', reward.referred_id).maybeSingle();
        return {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        {stats.recent_referrals.length > 0 && <Card>
            <CardHeader>
            ...
            <CardContent>
              <div className="space-y-4">
                {stats.recent_referrals.map((referral, index) => <div key={index} className="flex items-center justify-between border-b pb-3 last:border-0">
                    <div>
