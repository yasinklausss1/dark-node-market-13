...
export const ProductAddonsSelection = ({ productId, onSelectionsChange }: ProductAddonsSelectionProps) => {
  ...
  return (
    <div className="space-y-6 p-4 border rounded-lg bg-card">
      <h3 className="font-semibold text-lg">Produktoptionen</h3>
      ...
      <div className="space-y-4">
        {addons.map((addon) => (
          <div key={addon.id} className="flex items-start space-x-3 p-3 bg-muted/50 rounded-lg">
            <Checkbox
              id={addon.id}
              checked={selections.has(addon.id)}
              onCheckedChange={(checked) => toggleAddon(addon, checked as boolean)}
            />
